...
class ImageHelper
{
    ...
    public static function removeImagesFromField($model, $fieldName, $imagesToRemove)
    {
        if (!isset($model->$fieldName) || !is_array($imagesToRemove)) {
            return;
        }

        $existingImages = $model->$fieldName;
        ...
        foreach ($imagesToRemove as $img) {
            $relativePath = Str::after($img, 'storage');
            $relativePath = 'storage/' . ltrim($relativePath, '/');

            // Exact match only
            foreach ($existingImages as $existingImg) {
                if ($relativePath === $existingImg) {
                    if (file_exists($relativePath)) {
                        @unlink($relativePath);
                    }

                    $relativeToDelete[] = $relativePath;
                    break;
                }
            }
        }

        $remainingImages = array_values(array_diff($existingImages, $relativeToDelete));
        $model->$fieldName =  $remainingImages;
        $model->save();
    }
}
